...
class Q_net(nn.Module):
    def __init__(self, state_space=None,
                 action_space=None,bidirectional=False):
        ...
        self.num = 2 if self.bidirectional else 1
        self.Linear1 = nn.Linear(self.state_space, self.hidden_space)
        self.lstm = nn.LSTM(self.hidden_space, self.hidden_space, batch_first=True)
        self.Linear2 = nn.Linear(self.hidden_space * self.num, self.action_space)
    ...
    def init_b_state(self, batch_size, training=None):

        assert training is not None, "training step parameter should be dtermined"

        if training is True:
            return torch.zeros([1, batch_size, self.state_space])
        else:
            return torch.zeros([1, 1, self.state_space])
...
if __name__ == "__main__":
    ...
    for i in range(episodes):
        ...
        b_s = Q.init_b_state(batch_size=batch_size, training=False)

        for t in range(max_step):

            # Get action
            obs_tensor = torch.from_numpy(obs).float().unsqueeze(0).unsqueeze(0)
            a, h, c = Q.sample_action(obs_tensor.to(device),
